...
class RNG:
    """Utility class to wrap around jax.random functions.
    This simply lifts the requirement to keep track of the key seperately.
    """

    def __init__(self, seed: Optional[jnp.ndarray] = None, key: Optional[jnp.ndarray] = None):
        assert seed is not None or key is not None
        if seed is not None:
    ...
    def __getattr__(self, name):
        target_fn = getattr(jax.random, name)

        def fn(*args, **kwargs):
            self.key, subkey = jax.random.split(self.key)
            return target_fn(subkey, *args, **kwargs)
        return fn
...
class Logger:
    ...
    def delete(self):
        ...
        if len(path) < 40:
            print('NOT DELETING BECAUSE THE FOLDER PATH IS ONLY 40 CHARACTERS!')
        else:
            shutil.rmtree(path)
...
def get_pca_axis(coords: jnp.ndarray, weights: jnp.ndarray = None) -> Tuple[jnp.ndarray, jnp.ndarray]:
...
def make_fisher_trace(batch_network: Callable):
    """Creates a function that computes the trace of the Fisher information matrix for a given network.

    Args:
        batch_network (Callable): Network
    """
    def tree_dot(a, b):
    ...
    def fisher_trace(params, constants, electrons, atoms):
        def log_p_closure(p): return batch_network(
            p, constants, electrons, atoms)
        grads = jax.grad(lambda p: log_p_closure(p).mean())(params)
        return pmean_if_pmap(tree_dot(grads, grads))
    return fisher_trace
