...
#             weight = jax.ops.index_mul(weight, index, 1e-2)
#         else:
...
def update_parameter_initialization(
    ...
    if (jnp.mod(flat_original, 1) == 0).all():
        # We repalce the biases with the correct terms and set the weights to a
        # very small initialization such that their influence to the result is minor
        bias = jnp.ones_like(bias, dtype=bias.dtype) * flat_original.reshape(-1)[0]
        weight *= 1e-2
    else:
        total_weight = sum(weight_bias_ratio)
        weight_weight = jnp.sqrt(total_weight/weight_bias_ratio[0])
        bias_weight = jnp.sqrt(total_weight/weight_bias_ratio[1])
        base_std = bias.std()
        c_std = flat_original.std()
        weight *= c_std/weight_weight
        bias *= c_std/bias_weight
    return weight, bias
...
#     Args:
#         modeltype (str): indicating the model
...
#     n_atoms = len(charges)
#     if modeltype == 'ferminet':
#         model = FermiNet(len(charges),
#                          spins, 
#                          full_det, 
#                          envelope_type,
#                          **ferminet_params)
#     elif modeltype == 'fermi_dm':
#         model = Fermi_DM(spins, 
#                          full_det, 
#                          envelope_type,
#                          **ferminet_params)
...
#     batched_fermi = jax.vmap(model.apply, in_axes=(None, 0, None))
#     batch_batch_fermi = jax.vmap(batched_fermi, in_axes=(0, 0, 0))
#     batched_orbitals = jax.vmap(
#         functools.partial(model.apply, method=model.orbitals),
#         in_axes=(None, 0, None)
#     )
...
#         fermi_params = model.init(key, electrons, atoms).unfreeze()

#         # Extract node features
#         fermi_params, node_param_list, node_recover_fn = extract_from_tree(
#             fermi_params, node_filter, chunk_size=n_atoms)
#         n_params.append([p[None] for p in node_param_list])
        
#         # Extract global features
#         fermi_params, global_param_list, global_reocver_fn = extract_from_tree(
#             fermi_params, global_filter, chunk_size=None)
#         g_params.append([p[None] for p in global_param_list])

#     gnn = GNNFoo(n_params, g_params)
#     gnn_params = gnn.init(key, jnp.array([0], dtype=jnp.int32)).unfreeze()
...
#     def get_fermi_params(params, atoms, idx):
...
#     _batch_get_fermi_params = jax.vmap(get_fermi_params, in_axes=(None, 0, 0))
    
#     def batch_get_fermi_params(params, atoms):
#         """
#         atoms are batched
#         """
#         idx = jnp.arange(atoms.shape[0])
#         return _batch_get_fermi_params(params, atoms, idx)
    
#     def batch_pesnet(params, electrons, atoms):
#         """
#         e: n_configs x batchsize x 3N
#         a: n_configs x M x 3
#         """
#         fermi_params = batch_get_fermi_params(params, atoms)
#         return batch_batch_fermi(fermi_params, electrons, atoms)
...
#         ferminet=model,
#         gnn=gnn,
#         get_fermi_params=None,
#         batch_get_fermi_params=batch_get_fermi_params,
#         batch_pesnet=batch_pesnet,
...
def make_pesnet(
    ...
    **kwargs
    ...
    electrons = jnp.zeros((sum(spins)*3))
    key, subkey = jax.random.split(key)
    fermi_params = model.init(subkey, electrons, atoms).unfreeze()
    ...
    batch_get_fermi_params = jax.vmap(get_fermi_params, in_axes=(None, 0))
