...
from pesnet.constants import pmap, pmean_if_pmap
from pesnet.jnp_utils import tree_dot, tree_mul
...
def local_energy_diff(
    ...
    if method == 'ferminet':
        ...
        e_loc -= median
        if clip_local_energy > 0.0:
            tv = pmean_if_pmap(jnp.mean(jnp.abs(e_loc)))
            max_deviation = clip_local_energy*tv
            e_loc = jnp.clip(e_loc, -max_deviation, max_deviation)
        return e_loc
...
def make_loss_and_natural_gradient_fn(
    ...
    Args:
        ...
        aux_loss (Callable, optional): Auxilliary loss function. Defaults to None.
    ...
    def nat_cg(t, params, electrons, atoms, e_l, last_grad, damping):
        ...
        def loss(p):
            ...
            if aux_loss is not None:
                result += aux_loss(p, electrons, atoms)
            return result
        ...
        if center:
            ...
            mean_grads = pmean_if_pmap(mean_grads)
        ...
        def Fisher_matmul(v):
            w = v
            w = jax.jvp(log_p_closure, (params,), (w,))[1] / n
            # not sure about this one
            # https://github.com/netket/netket/blob/7d4d72ac5666dda2ac3bba31776fcb75e86668ff/netket/optimizer/sr/sr_onthefly_logic.py#L154
            # This is more noisy since we can't really pmean these statistics
            # if center:
            #     w = w - jnp.mean(w)
            uncentered = vjp_fn(w)[0]

            damp_vec = tree_mul(v, damping)

            result = jax.tree_multimap(
                lambda a, b: a + b, uncentered, damp_vec)
            if center:
                mg_v = tree_dot(mean_grads, v) / n
                result = jax.tree_multimap(
                    lambda a, b: a - mg_v*b, result, mean_grads)
            return result
        nat_grad = cg(
            A=Fisher_matmul,
            b=grad,
...
def make_training_step(
    ...
    Args:
        ...
        use_pmap (bool, optional): Whether to use pmap. Defaults to True.
    ...
    if use_pmap:
        # return pmap(step, donate_argnums=(1, 2, 4, 5))
        return pmap(step, donate_argnums=())
    else:
        return step
    ...
    for config in range(n_configs):
        ...
        if sum(charges) != n_electrons:
            ...
            while (sum(nalpha), sum(nbeta)) != spins:
                i = np.random.randint(0, len(nalpha))
            ...
            base_idx = jnp.concatenate([alpha_idx, beta_idx])
            atom_idx = jax.lax.broadcast(base_idx, (batch_size_per_config,))
