...
def scale_by_key(keys_and_scales: dict):
    """Optax transformation to scale different parameters differently.
    This only works for first-level parameters, e.g.,
    {
        'gnn': 0.1,
        'ferminet': 1.0
    }
    ...
    Args:
        keys_and_scales (dict): Dictionary with scaling factors.
    """
    def init_fn(_):
    ...
    def update_fn(updates, state, params=None):
        del params
        for key, scaling in keys_and_scales.items():
            updates[key] = jax.tree_map(lambda g: scaling*g, updates[key])
